...
LTHNHttpRequestInput::LTHNHttpRequestInput() {
    initialize();
}

LTHNHttpRequestInput::LTHNHttpRequestInput(QString v_url_str, QString v_http_method) {
    initialize();
    url_str = v_url_str;
    http_method = v_http_method;
}

void LTHNHttpRequestInput::initialize() {
    var_layout = NOT_SET;
    url_str = "";
    http_method = "GET";
}

void LTHNHttpRequestInput::add_var(QString key, QString value) {
    vars[key] = value;
}

void LTHNHttpRequestInput::add_file(QString variable_name, QString local_filename, QString request_filename, QString mime_type) {
    LTHNHttpFileElement file;
...
LTHNHttpRequestWorker::LTHNHttpRequestWorker(QObject *parent, QNetworkAccessManager *_manager)
...
LTHNHttpRequestWorker::~LTHNHttpRequestWorker() {
...
QMap<QString, QString> LTHNHttpRequestWorker::getResponseHeaders() const {
    return headers;
}

LTHNHttpFileElement LTHNHttpRequestWorker::getHttpFileElement(const QString &fieldname) {
    if (!files.isEmpty()) {
    ...
    return LTHNHttpFileElement();
}

QByteArray *LTHNHttpRequestWorker::getMultiPartField(const QString &fieldname) {
...
void LTHNHttpRequestWorker::setTimeOut(int timeOutMs) {
...
void LTHNHttpRequestWorker::setWorkingDirectory(const QString &path) {
    if (!path.isEmpty()) {
        workingDirectory = path;
    }
}

void LTHNHttpRequestWorker::setResponseCompressionEnabled(bool enable) {
    isResponseCompressionEnabled = enable;
}

void LTHNHttpRequestWorker::setRequestCompressionEnabled(bool enable) {
    isRequestCompressionEnabled = enable;
}

int  LTHNHttpRequestWorker::getHttpResponseCode() const{
    return httpResponseCode;
}

QString LTHNHttpRequestWorker::http_attribute_encode(QString attribute_name, QString input) {
...
void LTHNHttpRequestWorker::execute(LTHNHttpRequestInput *input) {
    ...
    if (input->var_layout == ADDRESS || input->var_layout == URL_ENCODED) {
        ...
        // add files
        for (QList<LTHNHttpFileElement>::iterator file_info = input->files.begin(); file_info != input->files.end(); file_info++) {
    ...
    if (input->request_body.size() > 0) {
        ...
        if(!isFormData && (input->var_layout != MULTIPART) && isRequestCompressionEnabled){
            request_content.append(compress(input->request_body, 7, LTHNCompressionType::Gzip));
        } else {
    ...
    if (LTHNHttpRequestWorker::sslDefaultConfiguration != nullptr) {
        request.setSslConfiguration(*LTHNHttpRequestWorker::sslDefaultConfiguration);
    }
...
void LTHNHttpRequestWorker::on_reply_finished(QNetworkReply *reply) {
    bool codeSts = false;
...
void LTHNHttpRequestWorker::on_reply_timeout(QNetworkReply *reply) {
...
void LTHNHttpRequestWorker::process_response(QNetworkReply *reply) {
...
QByteArray LTHNHttpRequestWorker::decompress(const QByteArray& data){
    
    Q_UNUSED(data);
    return QByteArray();
}

QByteArray LTHNHttpRequestWorker::compress(const QByteArray& input, int level, LTHNCompressionType compressType) {
...
QSslConfiguration *LTHNHttpRequestWorker::sslDefaultConfiguration;
